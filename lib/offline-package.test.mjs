...
test("ARM64 离线打包脚本为 Linux 和 macOS 生成统一命名的可移动运行包", (t) => {
  ...
  for (const directory of [
    ...
    join(packageRoot, ".next", "cache", "webpack"),
    join(packageRoot, ".next", "server"),
    ...
    join(packageRoot, ".python-runtime", "bin"),
    join(packageRoot, ".python-runtime", "lib", "python3.12", "site-packages"),
    join(packageRoot, ".python-runtime", "licenses"),
    join(packageRoot, ".python-runtime", "licenses", "runtime-components", "licenses"),
  ...
  writeFileSync(join(packageRoot, ".next", "cache", "webpack", "index.pack"), "build-cache\n");
  writeFileSync(join(packageRoot, ".next", "server", "app.js"), "production-server\n");
  ...
  writeFileSync(join(packageRoot, ".python-runtime", "bin", "python3"), "#!/bin/sh\n");
  writeFileSync(
    join(packageRoot, ".python-runtime", "lib", "python3.12", "site-packages", "INSTALLED_PACKAGES"),
    "python-pptx\nopenpyxl\npandas\n",
  );
  writeFileSync(join(packageRoot, ".python-runtime", "licenses", "Python-LICENSE.txt"), "Python license\n");
  writeFileSync(
    join(packageRoot, ".python-runtime", "licenses", "runtime-components", "PYTHON.json"),
    '{"target_triple":"aarch64-unknown-linux-gnu"}\n',
  );
  writeFileSync(
    join(packageRoot, ".python-runtime", "licenses", "runtime-components", "licenses", "LICENSE.openssl-3.txt"),
    "OpenSSL license\n",
  );
  writeFileSync(join(packageRoot, ".python-runtime", "licenses", "python-packages.json"), '{"installed":[]}\n');
  chmodSync(join(packageRoot, ".python-runtime", "bin", "python3"), 0o755);
  ...
  for (const platform of platforms) {
    ...
    const pythonPaths = [
      ...
      "runtime/python/lib/python3.12/site-packages/INSTALLED_PACKAGES",
      "runtime/python/licenses/Python-LICENSE.txt",
      "runtime/python/licenses/runtime-components/PYTHON.json",
      "runtime/python/licenses/runtime-components/licenses/LICENSE.openssl-3.txt",
      "runtime/python/licenses/python-packages.json",
    ];
    for (const relativePath of pythonPaths) {
      assert.equal(
        existsSync(join(bundleRoot, relativePath)),
        platform.includesPython,
        `${platform.name}: ${relativePath} Python packaging should match the target platform`,
      );
    }
    if (platform.includesPython) {
      assert.equal(readlinkSync(join(bundleRoot, "runtime", "bin", "python")), "../python/bin/python3");
      assert.equal(readlinkSync(join(bundleRoot, "runtime", "bin", "python3")), "../python/bin/python3");
    }
    assert.equal(
      existsSync(join(bundleRoot, "app", ".next", "cache")),
      false,
      `${platform.name}: app/.next/cache should not be packaged`,
    );
...
test("Linux ARM64 离线打包脚本明确拒绝缺失的 Python 运行时", (t) => {
  const testRoot = mkdtempSync(join(tmpdir(), "pi-web-offline-python-runtime-"));
  const packageRoot = join(testRoot, "project");
  const nodeArchive = join(testRoot, "node.tar.xz");
  const scriptPath = resolve("scripts/package-offline-arm64.sh");
  ...
  const result = spawnSync("bash", [scriptPath, "linux", nodeArchive, testRoot], {
    cwd: resolve("."),
    encoding: "utf8",
    env: {
      ...process.env,
  ...
  assert.match(result.stderr, /缺少 Linux ARM64 Python 运行时/);
});
