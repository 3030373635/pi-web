...
test("built-in subagents use registered persistent sessions with lightweight resources", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  ...
  assert.match(subagentSource, /noExtensions: true/);
  assert.match(subagentSource, /noSkills: true/);
  ...
  assert.match(startupSource, /createSubagentExtension\(SUBAGENT_CONTROLLER\.extensionRuntime, \(\) => listSubagentProfiles\(sessionCwd\)\)/);
...
test("existing-session prompts restore requested tools even when SSE created the wrapper first", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const routeSource = await readFile(new URL("../app/api/agent/[id]/route.ts", import.meta.url), "utf8");
  ...
  assert.match(promptSource, /requestedToolNames !== undefined/);
  assert.match(promptSource, /if \(requestedToolNames && !this\.isRunning\(\)\)/);
  assert.match(promptSource, /this\.applyActiveTools\(requestedToolNames\)/);
  assert.ok(promptSource.indexOf("this.applyActiveTools(requestedToolNames)") < promptSource.indexOf("this.inner.prompt("));
  assert.match(routeSource, /startRpcSession\(id, filePath, undefined, \{[\s\S]*?toolNames !== undefined/);
});
