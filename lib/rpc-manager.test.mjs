...
test("built-in subagents use registered persistent sessions with lightweight resources", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const subagentSource = source.slice(
    source.indexOf("async function startSubagent"),
    source.indexOf("export async function getSubagentRun"),
  );
  const startupSource = source.slice(source.indexOf("export async function startRpcSession"));
  ...
  assert.match(subagentSource, /registerRpcWrapper\(wrapper\)/);
  ...
  assert.match(startupSource, /createSubagentExtension\(SUBAGENT_EXTENSION_RUNTIME, \(\) => listSubagentProfiles\(sessionCwd\)\)/);
