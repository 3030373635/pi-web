...
test("completion notification waits for an accepted agent run to become idle", async (t) => {
  ...
  const wrapper = new AgentSessionWrapper(inner, (sessionId) => completed.push(sessionId));
  t.after(() => wrapper.destroy());
  wrapper.start();
...
test("an idle prompt applies its requested tool preset before SDK preflight", async (t) => {
  ...
  inner.agent.state.systemPrompt = "default prompt";
  inner.getAllTools = () => [
    { name: "read" },
    { name: "bash" },
    { name: "edit" },
  ...
  inner.setActiveToolsByName = (names) => {
    activeTools = names;
    inner.agent.state.systemPrompt = "rebuilt prompt";
  };
  const wrapper = new AgentSessionWrapper(inner);
  t.after(() => wrapper.destroy());

  await wrapper.send({ type: "prompt", message: "hello", toolNames: [] });
  await nextTurn();
